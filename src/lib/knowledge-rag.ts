...
const globalForEdukaPrisma = globalThis as unknown as { edukaPrisma?: PrismaClient };
export const edukaPrisma = globalForEdukaPrisma.edukaPrisma || new PrismaClient();
if (process.env.NODE_ENV !== "production") globalForEdukaPrisma.edukaPrisma = edukaPrisma;
...
export async function cacheKnowledgeSources(mode: KnowledgeMode, sources: KnowledgeSource[]) {
  for (const source of sources) {
    ...
    const doc = await edukaPrisma.knowledgeDocument.upsert({
    ...
    await edukaPrisma.knowledgeChunk.deleteMany({ where: { documentId: doc.id } });
    await edukaPrisma.knowledgeChunk.createMany({
...
export async function searchIndexedKnowledge(mode: KnowledgeMode, query: string): Promise<KnowledgeSource[]> {
  ...
  const chunks = await edukaPrisma.knowledgeChunk.findMany({
    where: { document: { mode } },
